...
namespace TeamDynamixLib {
    public class PrioritiesLib {
        ...
        /// <returns>A list of all active ticket priorities. (IEnumerable(Of TeamDynamix.Api.PriorityFactors.Priority)) </returns>
        public async Task<List<Priority>> GetTicketPrioritiesAsync(int appID, string authHeader, TDXEnvironment tDXEnvironment) {
            ...
            RestRequest restRequest = new RestRequest($"api/{appID}/tickets/priorities", Method.GET);
            ...
            return JsonConvert.DeserializeObject<List<Priority>>(response.Content);
        }
    }
}
